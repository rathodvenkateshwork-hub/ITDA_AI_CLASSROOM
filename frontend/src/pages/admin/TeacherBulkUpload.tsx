...
import { teacherBulkUpload } from "@/api/client";
...
const TeacherBulkUpload: React.FC = () => {
  ...
  const handleUpload = async () => {
    ...
    setError(null);
    try {
      const r = await teacherBulkUpload(rows);
  ...
  return (
    <DashboardLayout title="Bulk Upload Teachers">
      <div className="space-y-4">
        <p className="text-sm">
          Upload a CSV file with headers: <code>full_name,email,school_id,password</code> (password optional).
        </p>
